...
{
	/// <summary>
	/// 设备交互类的基类，实现了<see cref="IReadWriteDevice"/>接口的基础方法方法，需要使用继承重写来实现字节读写，bool读写操作。<br />
	/// The base class of the device interaction class, which implements the basic methods of the <see cref="IReadWriteDevice"/> interface, 
	...
	public class NetworkDeviceBase : NetworkDoubleBase, IReadWriteDevice
	{
		...
		///// 对设备来说，一个地址的数据对应的字节数，或是1个字节或是2个字节，通常是这两个选择
		///// </remarks>
		//protected virtual ushort GetWordLength( string address ) => 1;
		...
		/// <inheritdoc/>
		public override string ToString( ) => $"NetworkDeviceBase<{ GetNewNetMessage( ).GetType( )}, { ByteTransform.GetType( ) }>[{IpAddress}:{Port}]";

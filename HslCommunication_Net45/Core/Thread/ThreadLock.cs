	...
	public sealed class HslAsyncCoordinator
	{
		...
		public void StartOperaterInfomation()
		{
			...
			if (Interlocked.CompareExchange(ref OperaterStatus, 1, 0) == 0)
				ThreadPool.QueueUserWorkItem(new WaitCallback(ThreadPoolOperater), null);  // 启动保存
		}
		...
		private void ThreadPoolOperater(object obj)
		{
			...
			{
				startVal = currentVal;                                                        // 设置值
				action?.Invoke();                                                             // 以下为业务逻辑，允许实现非常复杂的设置
				currentVal = Interlocked.CompareExchange(ref Target, desiredVal, startVal);   // 需要清零值的时候必须用下面的原子操作
			}
			while (startVal != currentVal);                                                   // 更改失败就强制更新
			Interlocked.Exchange(ref OperaterStatus, 0);                                      // 退出保存状态
			if (Target != desiredVal) StartOperaterInfomation();                              // 最终状态确认
		}

		#region Private Member

		private Action action = null;               // 操作信息
		private int OperaterStatus = 0;             // 操作状态，0是未操作，1是操作中
		private long Target = 0;                    // 需要操作的次数
		...
		/// <inheritdoc/>
		public override string ToString( ) => "HslAsyncCoordinator";

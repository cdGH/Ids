...
namespace HslCommunication.Enthernet
{
	...
	public class NetComplexClient : NetworkXBase
	{
		...
		public void ClientClose( )
		{
			closed = true;
			if (IsClientStart)
				Send( session.WorkSocket, HslProtocol.CommandBytes( HslProtocol.ProtocolClientQuit, 0, Token, null ) );
		...
		private OperateResult<Socket> ConnectServer( )
		{
			...
			if (!sendResult.IsSuccess) return OperateResult.CreateFailedResult<Socket>( sendResult );
		...
		public void Send( NetHandle customer, string str ) { if (IsClientStart) Send( session.WorkSocket, HslProtocol.CommandBytes( customer, Token, str ) ); }
		...
		public void Send( NetHandle customer, byte[] bytes ) { if (IsClientStart) Send( session.WorkSocket, HslProtocol.CommandBytes( customer, Token, bytes ) ); }
		...
		private void ThreadHeartCheck()
		{
			...
			while (true)
			{
				...
				if (!closed)
				{
					...
					Send( session.WorkSocket, HslProtocol.CommandBytes( HslProtocol.ProtocolCheckSecends, 0, Token, send ) );
					double timeSpan = (DateTime.Now - session.HeartTime).TotalSeconds;
					if (timeSpan > 30d) // 30次没有收到失去联系
					{
						if (isConnecting == 0)
						{
		...
		private bool closed = false;                // 指示系统是否准备退出
		...
		/// <inheritdoc/>
		public override string ToString( ) => $"NetComplexClient[{EndPointServer}]";
